...
export default function AuthPage({ defaultMode = 'login' }: Props) {
  ...
  const playFlip = () => {
    try {
      ...
      for (let i = 0; i < d.length; i++) {
        ...
        const e =
          Math.exp(-t * 9) * Math.pow(Math.sin((Math.PI * t) / dur), 0.6)
  ...
  const handleRegister = async () => {
    setRegError('')
    if (!regName || !regPhone || !regDistrict || !regPassword) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: 'rgba(10,30,18,0.45)' }}
      />
      ...
      >
        ...
        <div
          className="absolute inset-y-0 right-0 z-[1] bg-white"
          style={{ width: '50%' }}
        />
...
function GreenBtn({
  ...
  return (
    <button
      ...
      onMouseOver={(e) => {
        if (!disabled) e.currentTarget.style.background = '#1e4a34'
      }}
      onMouseOut={(e) => {
        if (!disabled) e.currentTarget.style.background = '#2d5a45'
      }}
    >
      {children}
    </button>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      ...
      <input
        ...
        onFocus={(e) =>
          (e.target.style.boxShadow = '0 0 0 2px rgba(45,90,69,0.5)')
        }
